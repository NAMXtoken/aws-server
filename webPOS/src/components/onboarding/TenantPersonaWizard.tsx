...
import { persistTenantPersona } from '@/lib/onboarding/persist-persona'
...
type TenantPersonaWizardProps = {
    restrictToDev?: boolean
    title?: string
    description?: string
...
export default function TenantPersonaWizard({
    restrictToDev = false,
    title = 'Dev Tenant Onboarding',
    description = 'Quickly mint owner/sub-tenant personas for pager testing on localhost. Cookies update instantly; reload the POS app in a new tab to apply changes.',
    instructions,
    className,
    ctaHint,
}: TenantPersonaWizardProps) {
    ...
    const enabled = !restrictToDev || DEV_MODE_ENABLED
    ...
    const handleOwnerSubmit = useCallback(
        async (event: React.FormEvent<HTMLFormElement>) => {
            ...
            try {
                ...
                await persistTenantPersona({
                    mode: 'owner',
                    tenantId,
                    tenantSlug: baseSlug,
                    ownerEmail: trimmedEmail,
                    ownerName: ownerName.trim().length
                        ? ownerName.trim()
                        : null,
                })
    ...
    const handleSubSubmit = useCallback(
        async (event: React.FormEvent<HTMLFormElement>) => {
            ...
            await persistTenantPersona({
                ...
                tenantId,
                tenantSlug,
                subEmail: effectiveEmail,
                subName: subName.trim().length ? subName.trim() : null,
                subRole,
                pin: trimmedPin,
            ...
            setSubResult(
                `Linked to ${tenantSlug}. Sign in with ${effectiveEmail} to reach the PIN pad and continue as a ${subRole}.`
            )
        },
    ...
    if (!enabled) {
        return (
            <main
                className={`mx-auto flex max-w-2xl flex-col gap-4 px-4 py-10 ${className ?? ''}`}
            >
    ...
    const instructionsContent =
        instructions === undefined ? <DefaultInstructions /> : instructions

    return (
        <section
            className={`flex flex-col gap-6 rounded-2xl bg-white px-4 py-10 text-black shadow-xl shadow-slate-900/5 ring-1 ring-black/5 dark:bg-white/5 dark:text-white/90 dark:ring-white/10 sm:px-8 ${className ?? ''}`}
        >
            ...
            {mode === 'owner' ? (
                <section className="space-y-4 rounded-lg border border-emerald-300 bg-emerald-50 p-6 dark:border-emerald-500/40 dark:bg-emerald-500/10">
                    ...
                    <form className="space-y-4" onSubmit={handleOwnerSubmit}>
                        <div className="grid gap-2">
                            <label
                                htmlFor="owner-name"
                                className="text-sm font-medium text-emerald-700 dark:text-emerald-200"
                            >
                        ...
                        <div className="grid gap-2">
                            ...
                            <input
                                ...
                                placeholder="tenant-streetbrew"
                                className="rounded-md border border-emerald-200 bg-white px-3 py-2 text-black outline-none focus:border-emerald-400"
                            />
                        </div>
                        <button
                            type="submit"
                            className="w-full rounded-md bg-emerald-600 px-4 py-2 text-white shadow hover:bg-emerald-500"
                        >
                            Create tenant and invite
                        </button>
                        {ownerResult ? (
                            <div className="rounded-md bg-white/80 p-4 text-sm text-gray-800">
                                ...
                                <p className="mt-2 text-xs text-gray-500">
                                    Share this code with sub-tenants. They must
                                    sign in with the email you provided to reach
                                    the PIN screen.
                                </p>
                            </div>
                        ) : null}
            ...
            {mode === 'subtenant' ? (
                <section className="space-y-4 rounded-lg border border-sky-300 bg-sky-50 p-6 dark:border-sky-500/40 dark:bg-sky-500/10">
                    <h2 className="text-xl font-semibold text-sky-700 dark:text-sky-200">
                        Sub-tenant setup
                    </h2>
                    <form className="space-y-4" onSubmit={handleSubSubmit}>
                        <div className="grid gap-2">
                            <label
                                htmlFor="invite-code"
                                className="text-sm font-medium text-sky-700 dark:text-sky-200"
                            >
                                Invite code
                            </label>
                            <div className="flex flex-col gap-2 sm:flex-row">
                                <input
                                    ...
                                    placeholder="bynd-dev:tenant-..."
                                    className="flex-1 rounded-md border border-sky-200 bg-white px-3 py-2 text-black outline-none focus:border-sky-400"
                                />
                                {inviteOptions.length ? (
                                    ...
                                    >
                                        <option value="">Load invite</option>
                        ...
                        <div className="grid gap-2">
                            ...
                            <p className="text-xs text-sky-700/80 dark:text-sky-200/80">
                                You will need this email to sign in before
                                reaching the PIN screen.
                            </p>
                        </div>
                        <div className="grid gap-2">
                            <label
                        ...
                        <button
                            type="submit"
                            className="w-full rounded-md bg-sky-600 px-4 py-2 text-white shadow hover:bg-sky-500"
                        >
                            Join tenant
                        </button>
                        {subResult ? (
                            <p className="rounded-md bg-white/80 p-4 text-sm text-gray-800">
                                {subResult}
                            </p>
                        ) : null}
                        {subError ? (
